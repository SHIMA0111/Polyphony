...
6. [ ] Multi-instance cross-delivery check via `task verify:redis-hub` against a real two-replica compose stack. (Wave-4 integration review: the *substance* passes — with a locally patched override, a message POSTed to replica 2 was delivered over replica 1's WebSocket via RedisHub — but `task verify:redis-hub` itself fails as committed: (a) `docker-compose.scale-test.yml`'s `ports: ["8080"]` is *merged* with, not substituted for, the base `"8080:8080"` mapping, so scaled replicas still collide on host port 8080 — it needs `ports: !override`; (b) the script's `docker compose up -d --wait db migrate redis llm-gateway` exits 1 whenever an exited `migrate` container pre-exists, and its cleanup trap then downs whatever stack was running. See the wave-4 review findings.)
