...
## Verification
1. `cd server && go build ./... && go vet ./...` — compiles clean with the new `redis` dependency and config fields.
2. `cd server && go test ./...` — all existing unit tests plus the new `config_test.go`/`RedisHub` unit tests pass.
3. `cd server && go test -tags=integration ./internal/infrastructure/event/...` (adjust the build tag to match the convention Step 2 established for Postgres integration tests) — the two-instance Redis Pub/Sub integration test passes, including the per-user targeting assertion.
4. `docker compose up -d db migrate redis llm-gateway` then `docker compose ps` — `redis` shows `healthy`.
5. `docker compose up -d api` (single instance, default compose) then `curl -s http://localhost:8080/health` returns 200, and `docker compose logs api | grep "message hub driver selected"` shows `driver=redis`.
6. Multi-instance cross-delivery check:
   - `docker compose -f docker-compose.yml -f docker-compose.scale-test.yml up -d --scale api=2`
   - `docker compose port --index=1 api 8080` and `docker compose port --index=2 api 8080` to get each replica's host port.
   - Run `task verify:redis-hub` (or the underlying script directly, passing the two ports) — it registers a user, opens a WebSocket against replica 1's port, sends a room message via HTTP against replica 2's port, and must observe the message delivered over the replica-1 WebSocket within the script's timeout. The script exits 0.
   - `docker compose -f docker-compose.yml -f docker-compose.scale-test.yml down` cleans up afterward.
7. Restart with `MESSAGE_HUB_DRIVER=inprocess docker compose up -d api` (single instance) or `cd server && go run ./cmd/api` with only `DATABASE_URL`/`JWT_SECRET` set (no `REDIS_URL`) — server starts successfully with `driver=inprocess` logged, confirming the default/no-Redis path still works for plain local dev.
8. `task test` and `task lint` at the repo root still pass end-to-end (no regressions introduced in other components).

## Completion criteria
- [ ] `RedisHub` implements the Step 15 `MessageHub` interface with per-room channels and per-user (`TargetUserID`) filtering.
- [ ] `MESSAGE_HUB_DRIVER` env flag selects between `InProcessHub` (default) and `RedisHub` with no other code path changes.
- [ ] `docker-compose.yml` has a healthchecked `redis:7-alpine` service and `api` depends on it when the redis driver is the compose default; `.env.example` documents the new vars.
- [ ] `docker-compose.scale-test.yml` override exists and enables `--scale api=2` without host-port collisions.
- [ ] Unit tests (config validation, RedisHub construction) and the testcontainers-backed integration test (two-instance pub/sub + per-user targeting) are added and pass.
- [ ] The multi-instance WebSocket delivery smoke check (`task verify:redis-hub` or equivalent script) passes against a real two-replica compose stack.
- [ ] All items in the Verification section pass.
- [ ] No changes to `schema.sql`, Atlas migrations, or the `event.MessageHub`/`Event` type definitions themselves.
