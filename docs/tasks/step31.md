...
4. [ ] `docker compose up -d db migrate redis llm-gateway` then `docker compose ps` — `redis` shows `healthy`. (Skipped in this worktree: requires the full compose stack on fixed host ports; left for post-merge integration review.)
5. [ ] `docker compose up -d api` (single instance, default compose) then `curl -s http://localhost:8080/health` returns 200, and `docker compose logs api | grep "message hub driver selected"` shows `driver=redis`. (Skipped — same reason as above.)
6. [ ] Multi-instance cross-delivery check via `task verify:redis-hub` against a real two-replica compose stack. (Script written at `server/scripts/verify_redis_hub.sh` and wired to the Taskfile target, but not executed end-to-end in this worktree — skipped per the full-compose-stack restriction; left for post-merge integration review.)
7. [ ] Restart with `MESSAGE_HUB_DRIVER=inprocess docker compose up -d api` ... confirming the default/no-Redis path still works. (Skipped — same reason; the equivalent `go run ./cmd/api` local-dev path with no `REDIS_URL`/`MESSAGE_HUB_DRIVER` set is exercised indirectly by `TestNewContainerBadDatabaseURL`, which relies on `MessageHubDriver` defaulting to `"inprocess"`.)
8. [ ] `task test` and `task lint` at the repo root still pass end-to-end. (Ran `task test:server`/`lint:server` equivalents (`go build`, `go vet`, `go test ./...`, `golangci-lint run ./...`) directly; did not run the full multi-language `task test`/`task lint` in this pass — see skippedComposeChecks/summary.)
...
- [ ] The multi-instance WebSocket delivery smoke check (`task verify:redis-hub` or equivalent script) passes against a real two-replica compose stack. (Written, not executed — see above.)
- [ ] All items in the Verification section pass. (Items 1-3 pass; items 4-8 require the full compose stack and are left for post-merge integration review.)
