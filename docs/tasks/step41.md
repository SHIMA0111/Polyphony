...
- [x] Update `List`/`Send` handlers and any other `usecase` call sites to pass the authenticated user ID as the new `requestingUserID` repository/usecase argument (the handler already has `userID := middleware.GetUserID(c)` available).
- [x] Tests:
...
- [x] `messages.visibility` column exists in `server/schema.sql` with a migration generated via `task migrate:generate` and applied successfully. (Applied successfully against the testcontainers harness; not applied against the shared compose db — see Verification item 3.)
- [x] `MessageVisibility` type/constants and `Message.Visibility` field added per existing entity conventions.
- [x] `ListByRoom`, `GetByID`, `ListByRoomUpTo` (interface, Postgres implementation, and all usecase call sites) filter out other users' private messages.
- [x] `SendAIMessage` supports a `private` flag that marks both the human and AI message rows private, with the AI message's `SenderID` set to the requester for filtering purposes (documented in code).
- [x] `RegenerateAIMessage` returns `domain.ErrForbidden` when a non-owner targets another user's private exchange.
- [x] Private message WS events are delivered only to the sender via the `MessageHub`'s per-user targeted delivery; public messages remain room-broadcast.
