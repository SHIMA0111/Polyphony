...
1. [ ] `docker compose up -d dex` then `docker compose ps dex` — status is `healthy`. *(needs the compose stack; not run — see note above. Standalone equivalent run: a bare `docker run dexidp/dex:v2.43.1 dex serve` with this exact `ory/dex/config.yaml` started cleanly and its healthcheck command succeeded.)*
...
3. [ ] `docker compose up -d kratos` ... lists `"dex"`, `"google"`, `"github"`. *(needs the compose stack; not run. Standalone equivalent: a bare Kratos container with this exact `kratos.yml` + `SELFSERVICE_METHODS_OIDC_CONFIG_PROVIDERS` (dex entry only, since google/github need no live verification) reported the `dex` oidc node on `GET .../registration/browser`.)*
4. [x] `cd web && bun install && bunx playwright install --with-deps chromium` — ran; exits 0.
5. [ ] `task test:e2e:up` then `bunx playwright test oauth-dex.spec.ts`. *(needs the compose stack; not run — see `skippedComposeChecks`.)*
6. [ ] Kratos admin API identity-count check. *(needs the compose stack; not run. Standalone equivalent: after two full authorization-code logins with the fixture user against a standalone Kratos+dex pair, `admin/identities` reported exactly 1 identity for the fixture email.)*
...
8. [ ] Account-linking check. *(needs the compose stack; not run. Standalone equivalent: after the standalone dex-created identity above existed, `POST .../self-service/registration/api` with `method: password`, `traits.email` equal to the same fixture email, and a different password returned a `4000007` "account with the same identifier ... exists already" error, confirming no silent merge.)*
9. [x] `bun run lint` in `web/` — 0 errors, 4 warnings (all pre-existing, unrelated to this step's files).
10. [ ] `task test:e2e:down` then `docker compose down`. *(needs the compose stack; not run.)*
...
- [ ] All verification checks above pass — compose-dependent checks (1, 3, 5, 6, 8, 10) were not run in this isolated-worktree pass; every check runnable without the compose stack passed.
