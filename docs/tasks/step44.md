...
5. [x] Verified in the wave-5 integration review: `bunx playwright test oauth-dex.spec.ts` against the `task test:e2e:up` stack — both serial cases pass. (A follow-up review round found the e2e project's `dex` published host port 5556 collided with a running dev stack's `dex`; fixed by giving the e2e project its own `dex-e2e` service on host port 8095, aliased as `dex` inside the e2e project's own network — see docker-compose.yml/Taskfile.yml/web/playwright.config.ts. `task test:e2e:up` now runs fully alongside a running dev stack with no manual stop-the-dev-dex step required.)
...
- [x] All verification checks above pass — the compose-dependent checks (1, 3, 5, 6, 8, 10) were completed in the wave-5 integration review. The e2e/dev `dex` host-port collision found in that review (see item 5 above) was fixed with a dedicated `dex-e2e` service; `task test:e2e:up` no longer requires stopping the dev stack's `dex` first.
