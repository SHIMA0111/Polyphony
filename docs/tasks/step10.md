...
7. [ ] `task test:e2e` — **RAN — FAIL (post-merge integration review), two defects found: (a) Playwright's Node-based loader cannot import `e2e/seed/seed.ts` as `globalSetup` — the file uses Bun's `import.meta.main`, and with no `"type": "module"` in `web/package.json` Playwright transpiles it to CJS, where `import.meta` is a syntax error, so every `bunx playwright test` run aborts before any spec executes; (b) with `globalSetup` bypassed, `smoke.spec.ts` fails a strict-mode violation — `getByRole('button', { name: 'New Room' })` matches two buttons on the empty rooms page (header + empty-state CTA). With `.first()` applied to that one locator, the full register → create room → send message flow passes against the test stack.**
8. [x] `task test:e2e:down` — **RAN (post-merge integration review): exits 0 and removes the e2e services/volumes, BUT `docker compose --profile test down -v` is not scoped to the profile — it also stopped the dev-stack services and deleted the dev `pgdata`/`kratos_pgdata` volumes. Reported as a review issue (isolate the test stack with a dedicated compose project name, e.g. `docker compose -p polyphony-e2e`).**
...
- [ ] The smoke spec passes against the Step-4 BFF proxy path (register → create room → send message). **FAILS as shipped** — `task test:e2e` aborts on the `seed.ts` `import.meta.main`/CJS `globalSetup` load error, and the spec itself has an ambiguous "New Room" locator (strict-mode violation). The flow passes end to end once those two defects are fixed (verified in the post-merge integration review with a locally patched copy); see Verification #7.
- [ ] All verification checks above pass. **All run in the post-merge integration review; #7 fails (two defects, see above), everything else passes.**
