...
## Scope
- [x] Add `@playwright/test` as a dev dependency in `web/package.json` (via `bun add -d @playwright/test`) and a `web/playwright.config.ts` with `testDir: "./e2e"`, a `baseURL` read from `PLAYWRIGHT_BASE_URL` (default `http://localhost:3001`), a `globalSetup` pointing at the seed script, and a single `chromium` project.
- [x] Create `web/e2e/support/fixtures.ts` exporting the fixed E2E fixture user (`email`, `username`, `password`) and fixture room name as constants, for this and future spec steps to import.
- [x] Create `web/e2e/seed/seed.ts`: an idempotent seed routine (default-exported `globalSetup` function, plus a `bun run` entrypoint guard) that waits for the API's `/health`, registers the fixture user (falling back to login if the user already exists — HTTP 400/409 from `/auth/register`), and ensures the fixture room exists (list rooms, create if missing by name) via direct `fetch` calls to the test-profile API (`E2E_API_URL`, default `http://localhost:8090`). Must be safe to run repeatedly with no side effects on the second+ run.
- [x] Create `web/e2e/smoke.spec.ts`: a single Playwright spec that registers a brand-new user (unique email/username per run, no seed dependency), creates a new room, opens it, sends a plain (non-AI) message, and asserts the message appears — driven entirely through the UI so it exercises the Step-4 BFF proxy end to end.
- [x] Add Playwright artifacts (`web/playwright-report/`, `web/test-results/`, `web/e2e-report/`) to `web/.gitignore`.
- [x] Create a new `llm-stub/` service: a minimal Bun HTTP server (`llm-stub/server.ts`, `llm-stub/package.json`, `llm-stub/Dockerfile`) implementing an OpenAI-compatible `POST /v1/chat/completions` (matching the shape `OpenAIProvider` in `llm-gateway/src/adapters/outbound/openai.rs` sends/expects) plus `GET /health`.
- [x] Implement deterministic fixture selection in the stub: if the last user message starts with `[[fixture:NAME]]`, load `llm-stub/fixtures/NAME.json` (non-streaming) or `llm-stub/fixtures/stream/NAME.sse` (when the request body has `"stream": true`); otherwise use `default`. Unknown fixture names return HTTP 404 with a JSON error body so a missing fixture fails a spec loudly instead of silently returning the wrong canned answer.
- [x] Add `llm-stub/fixtures/default.json` (canned non-streaming completion body) and `llm-stub/fixtures/stream/default.sse` (canned OpenAI-style SSE chunk sequence terminated by `data: [DONE]`), matching OpenAI's real wire formats so later steps can add more `NAME.json` / `NAME.sse` fixtures without touching `server.ts`.
- [x] Add a minimal `llm-stub/server.test.ts` (run via `bun test`) covering the fixture-selection logic (default fallback, explicit `[[fixture:NAME]]` marker, unknown-fixture 404) and the streaming vs. non-streaming branch.
- [x] Add a Docker Compose **test profile** to `docker-compose.yml`: new services `api-e2e`, `db-e2e`, `llm-gateway-e2e`, `llm-stub`, `migrate-e2e`, `web-e2e`, each tagged `profiles: ["test"]`, on alternate host ports (`5433`, `8090`, `8091`, `8092`, `3001`) so the test stack can run alongside the normal dev stack without conflicts. `llm-gateway-e2e` sets `OPENAI_BASE_URL` to the internal `llm-stub` address so the gateway talks to the stub instead of the real OpenAI API.
- [x] Wire `web-e2e`'s server-side proxy target (the internal base-URL env var Step 4's `app/api/proxy/[...path]` route reads) at `http://api-e2e:8080`, matching whatever env var name Step 4 introduced (inspect the proxy route file — see Implementation notes).
- [x] Add Taskfile tasks: `test:e2e:up` (start the test profile, build if needed), `test:e2e:seed` (run the seed script standalone against the test stack), `test:e2e` (ensure the stack is up, then run Playwright — seeding also happens automatically via `globalSetup`), and `test:e2e:down` (tear the test stack down, including its volumes).
- [x] Update `README.md` (or add a short section) documenting how to run the E2E stack locally, kept minimal — full docs/progress sync happens in the final gate step.
...
## Verification
1. [x] `cd web && bun install && bunx playwright install --with-deps chromium` — installs the Playwright browser; exits 0.
2. [x] `cd llm-stub && bun test` — the fixture-selection unit tests pass. (11 tests passed.)
3. [x] `task test:e2e:up` — brings up the six test-profile services; `docker compose --profile test ps` shows `db-e2e`, `migrate-e2e` (exited 0), `api-e2e`, `llm-stub`, `llm-gateway-e2e`, `web-e2e` all `Up`/healthy. **RAN — PASS (post-merge integration review): all six services came up healthy (migrate-e2e Exited 0) alongside the running dev stack.**
4. [x] `curl -s http://localhost:8092/health` → `{"status":"ok"}` (HTTP 200). **RAN — PASS (post-merge integration review) against the compose test profile; the gateway-e2e → llm-stub completion path also returned the canned fixture via `POST http://localhost:8091/completions`.**
5. [x] `curl -s -X POST http://localhost:8091/completions ...` → verified equivalently by running `llm-gateway` (`OPENAI_BASE_URL` pointed at a locally-run `llm-stub`) and `llm-stub` directly as local processes on arbitrary ports (not via compose) and curling the gateway's actual `POST /completions` path: response was `{"id":"chatcmpl-e2e-default","model":"gpt-5-mini","choices":[{"index":0,"message":{"role":"assistant","content":"This is a canned E2E stub response for testing purposes."},...}],...}`, confirming the gateway → stub wiring end to end.
6. [x] `task test:e2e:seed` run twice in a row — **RAN — PASS (post-merge integration review): both runs printed "E2E seed complete."; the test DB afterwards held exactly one fixture user and one fixture room (idempotency confirmed).**
7. [x] `task test:e2e` — **RAN — PASS (round-2 integration re-review, after the "Wave 2 review fixes (round 1)" commit).** Round 1 had found two defects: (a) Playwright's Node-based loader could not import `e2e/seed/seed.ts` as `globalSetup` (Bun-only `import.meta.main` guard, CJS transpile → syntax error) — fixed by moving the CLI entrypoint to `e2e/seed/cli.ts`; (b) `smoke.spec.ts` hit a strict-mode violation on the ambiguous "New Room" locator — fixed with `.first()`. Re-run end to end against the freshly built `-p polyphony-e2e` test stack: `globalSetup` seed loads and runs under Playwright, and the full register → create room → send message spec passes (1 passed).
8. [x] `task test:e2e:down` — **RAN (post-merge integration review): exits 0 and removes the e2e services/volumes, BUT `docker compose --profile test down -v` is not scoped to the profile — it also stopped the dev-stack services and deleted the dev `pgdata`/`kratos_pgdata` volumes. Reported as a review issue (isolate the test stack with a dedicated compose project name, e.g. `docker compose -p polyphony-e2e`). FIXED in "Wave 2 review fixes (round 1)": `test:e2e:up`/`test:e2e:down` now run under `-p polyphony-e2e`; re-verified in the round-2 re-review — `down -v` removed only `polyphony-e2e_*` containers/volumes/network.**
9. [x] `task up` — **RAN — PASS (post-merge integration review): the full dev stack (api, db, kratos, kratos-db, kratos-migrate, llm-gateway, mailslurper, migrate, web) came up healthy from a fresh build, concurrently with the test profile.**
10. [x] `task test:server && task test:gateway` — existing unit tests still pass unmodified (regression check that no Go/Rust source was touched). Ran via `task test:server && task test:gateway`: all Go and Rust tests pass.

## Completion criteria
- [x] `web/playwright.config.ts` and `web/e2e/` (smoke spec, seed script, fixtures support module) exist and are wired to run against the test-profile stack.
- [x] `llm-stub/` service exists with deterministic fixture-based completions and SSE streaming responses, plus its own unit tests.
- [x] `docker-compose.yml` has a working, alternate-port `test` profile covering all six new services, fully additive to the existing dev services (`docker compose --profile test config` validated statically).
- [x] `Taskfile.yml` has `test:e2e:up`, `test:e2e:seed`, `test:e2e`, `test:e2e:down` tasks.
- [x] The seed routine is idempotent by construction (register-then-fallback-to-login, list-then-create-room-only-if-missing); exercised against the live test stack in the post-merge integration review (Verification #6 — two consecutive runs, exactly one fixture user/room).
- [x] The smoke spec passes against the Step-4 BFF proxy path (register → create room → send message). Originally failed as shipped (the `seed.ts` `import.meta.main`/CJS `globalSetup` load error and the ambiguous "New Room" locator); both defects were fixed in "Wave 2 review fixes (round 1)" and the spec now passes end to end against the compose test stack (round-2 re-review); see Verification #7.
- [x] All verification checks above pass. **All re-run/confirmed in the round-2 integration re-review after the round-1 fixes; #7 now passes.**
