...
- [x] All verification checks above pass. (1-4 run and pass in the implementation worktree; 5-6 run and pass in the post-merge integration review against the compose test profile at `http://localhost:3001` — initial `/rooms` and `/rooms/[roomId]` HTML already contained the dehydrated room list/models (RSC prefetch confirmed via cookie-authenticated `curl` + grep), and creating a room, sending a plain message, and sending an AI message all updated the UI without a page reload. Caveat found during review: with a *present but invalid* session cookie, `apiFetch`'s 401 `window.location.assign("/login")` and the middleware's presence-only cookie check bounce the browser between `/login` and `/rooms` in an infinite reload loop — reported as a review issue.)
