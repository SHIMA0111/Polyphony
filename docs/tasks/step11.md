...
## Scope
- [ ] Create `ory/kratos/kratos.yml` — Kratos configuration: `dsn` from `KRATOS_DSN` env var, `serve.public.base_url` / `serve.admin.base_url`, `secrets.cookie` / `secrets.cipher` from env, `identity.default_schema_id: default` + `identity.schemas` pointing at the mounted identity schema file, `selfservice.default_browser_return_url` and `allowed_return_urls` pointing at placeholder `http://localhost:3000/*` routes, `selfservice.methods.password.enabled: true`, `selfservice.flows.registration|login|settings|recovery|verification|logout` each with a placeholder `ui_url` under `http://localhost:3000/...` (the real web pages land in Step 20/44), and `courier.smtp.connection_uri` pointing at the `mailslurper` service.
- [ ] Create `ory/kratos/identity.schema.json` — JSON Schema for the default identity: `traits.email` (format `email`, used as the password identifier, `verification.via: email`, `recovery.via: email`) and `traits.username` (string, 3-100 chars), both `required`, mirroring the `email`/`username` columns already on the `users` table in `server/schema.sql`.
- [ ] Add `kratos-db` service to `docker-compose.yml` — `postgres:17-alpine`, dedicated named volume, its own `POSTGRES_USER`/`POSTGRES_PASSWORD`/`POSTGRES_DB` (`KRATOS_DB_*` env vars), `pg_isready` healthcheck matching the existing `db` service's pattern.
- [ ] Add `kratos-migrate` service to `docker-compose.yml` — one-shot job using the same Kratos image tag as the `kratos` service, running `kratos migrate sql -e --yes`, `depends_on: kratos-db` with `condition: service_healthy`, mounts `./ory/kratos:/etc/config/kratos:ro`.
- [ ] Add `kratos` service to `docker-compose.yml` — runs `kratos serve -c /etc/config/kratos/kratos.yml --dev`, mounts `./ory/kratos:/etc/config/kratos:ro`, exposes `4433:4433` (public) and `4434:4434` (admin), `depends_on: kratos-migrate` with `condition: service_completed_successfully`, healthcheck against `GET /health/ready` on the admin port.
- [ ] Add `mailslurper` service to `docker-compose.yml` — `oryd/mailslurper:latest-smtps`, exposes `4436:4436` (web UI) and `4437:4437` (REST API used by the UI and by verification below); no host port needed for the internal SMTP listener (1025) since only `kratos` talks to it over the compose network.
- [ ] Insert the four new blocks into `docker-compose.yml` in alphabetical position per the Step 6 convention, so the final service order is: `api`, `db`, `kratos`, `kratos-db`, `kratos-migrate`, `llm-gateway`, `mailslurper`, `migrate`, `web`.
- [ ] Add a `# === Ory Kratos ===` section to `.env.example` (and mirror into local `.env`) with `KRATOS_DB_USER`, `KRATOS_DB_PASSWORD`, `KRATOS_DB_NAME`, `KRATOS_DSN`, `KRATOS_PUBLIC_URL`, `KRATOS_ADMIN_URL`, `KRATOS_COOKIE_SECRET`, `KRATOS_CIPHER_SECRET` (cipher secret must be exactly 32 characters — document this constraint inline as a comment).
- [ ] Document the new services (ports, purpose, how to reach the Mailslurper UI, how to inspect identities via the admin API) in a short `ory/README.md`.
- [ ] Verify no existing service definition (`db`, `migrate`, `api`, `llm-gateway`, `web`) is modified — this PR only adds new blocks and new files.
...
## Completion criteria
- [ ] `ory/kratos/kratos.yml` and `ory/kratos/identity.schema.json` exist and are checked in.
- [ ] `ory/README.md` documents the new services, ports, and how to inspect identities/emails.
- [ ] `docker-compose.yml` gains `kratos`, `kratos-db`, `kratos-migrate`, `mailslurper` as new, alphabetically-positioned, additive blocks; no existing service block is modified.
- [ ] `.env.example` (and local `.env`) document all new `KRATOS_*` variables with clear comments, including the 32-character cipher secret constraint.
- [ ] No files under `server/`, `llm-gateway/`, or `web/` are modified by this PR.
- [ ] All verification checks above pass locally against a clean `docker compose down -v && docker compose up -d`.
