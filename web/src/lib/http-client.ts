 ...
 *   (dead) session cookie via `POST /api/auth/logout` and then redirects the
 *   browser to `/login` (guarded by `typeof window !== "undefined"` so this
 *   is a no-op during SSR), since a 401 means the session cookie is missing,
 ...
 *   landing on the login page. `skipAuthRedirect` is set by
