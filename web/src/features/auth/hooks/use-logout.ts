...
/**
 * Logs the user out via Kratos's self-service logout flow, clears the
 * entire React Query cache, and navigates to `/login`.
 *
 ...
 * happen regardless of whether the logout call itself succeeded or failed: a
 * failed logout request must not strand the user on the current page, since
 * the intent (leave this session) still applies even if the server-side call
 * errored.
 */
