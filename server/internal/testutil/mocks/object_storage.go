...
type ObjectStorage struct {
	...
	PresignUploadFunc func(ctx context.Context, key, contentType string, contentLength int64, expires time.Duration) (string, error)
...
func (s *ObjectStorage) PresignUpload(ctx context.Context, key, contentType string, contentLength int64, expires time.Duration) (string, error) {
	if s.PresignUploadFunc != nil {
		return s.PresignUploadFunc(ctx, key, contentType, contentLength, expires)
	}
	if s.ShouldErr {
