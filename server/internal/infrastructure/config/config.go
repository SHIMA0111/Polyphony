...
type Config struct {
	...
	// "http://localhost:3000/billing/checkout/success"). This is stored and
	// passed through verbatim, without a "session_id" query parameter —
	// usecase/billing.BillingUsecase appends Stripe's own
	// "session_id={CHECKOUT_SESSION_ID}" template placeholder to it at
	// Checkout Session creation time, so any operator-configured value here
	// (with or without its own query string) works without needing to know
...
func Load() (*Config, error) {
	...
	if v := os.Getenv("LLM_GATEWAY_GRPC_MAX_RETRIES"); v != "" {
		...
		// A negative value parses successfully as an int but is not a valid
		// retry count (GRPCClient.callWithRetry only guards maxRetries < 1 by
		// clamping to 1, so a negative value would silently behave like 1
		// attempt with no indication anything was wrong) — treat it the same
		// as a parse failure: warn and keep the default.
		if err == nil && n < 0 {
			err = fmt.Errorf("must be non-negative, got %d", n)
		}
		if err != nil {
